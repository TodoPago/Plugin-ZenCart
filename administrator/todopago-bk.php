...
<body onLoad="init()">
    ...
    $mediosDePagos = $row['medios_pago'] ? json_decode($row['medios_pago'], true) : array();
...
<div id="todopago">
  ...
  <div id="config">  
        ...
        <label>Authorization HTTP (código de autorización)</label>
        <input type="text" value='<?php echo(isset($jsonAuthorization->Authorization)? $jsonAuthorization->Authorization:"")?>' placeholder="Authorization HTTP" name="authorization"/>
        </div>
...
<div id="mediosdepago">
    ...
    <?php else : ?>
    <?php foreach ($paymentMethods['PaymentMethod'] as $mp): ?>
    <?php 
        $checked = array_key_exists($mp['ID'], $mediosDePagos)
            ? $mediosDePagos[$mp['ID']] == 'true'? 'checked': ''
            :'checked';
    ?>
    <div class="input-todopago">
        <label for="<?php echo $mp['Name'] ?>"><img src="<?php echo $mp['Logo'] ?>" title="<?php echo $mp['Name'] ?>" style="height: 25px" /> <?php echo $mp['Name'] ?></label>
        <div style="text-align: center;">
            <input type="checkbox" name="medios_pago[<?php echo $mp['ID'] ?>]" id="<?php echo $mp['Name'] ?>" class="mediodepago_checkbox" value="<?php echo $mp['ID'] ?>" <?php echo $checked ?> />
        </div>
    </div>
    <?php endforeach ?>
    <?php endif ?>
    <input type="hidden" name="medios_pago" class="medios_pago" value="0" />
    <div class="margin-superior"><input  onclick="submitDataTodoPago()" type="submit" name="submit" value="Guardar Datos"/></div>    
</form>
</div>
<div id="prod">
...
$('#orders-table').dataTable(
  ...
  $(".tabs-todopago").click(function(){
    ...
    function submitDataTodoPago(){
        var mediosdepago = {};
        $('.mediodepago_checkbox').each(function(index, element){
           mediosdepago[$(element).val()] = $(element).is(":checked");
        });
        $('.medios_pago').val(JSON.stringify(mediosdepago));
        $('#form').submit();
    }  
  </script>
</body>
